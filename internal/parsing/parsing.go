...
func NormalizeDriveCategory(text string, mediaType *domain.MediaType) *domain.DriveCategory {
	...
	// ⚡ Bolt: Replaced sequential strings.ReplaceAll calls with a single-pass strings.Builder iteration
	// to eliminate intermediate string allocations and improve parsing performance.
	var b strings.Builder
	b.Grow(len(folded))
