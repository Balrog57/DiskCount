...
func NormalizeDriveCategory(text string, mediaType *domain.MediaType) *domain.DriveCategory {
	...
	// ⚡ Bolt: Use byte-level iteration to avoid multiple string allocations from strings.ReplaceAll
	// while still preserving asciiFold logic that correctly handles non-ascii bytes
	var b strings.Builder
	b.Grow(len(folded))
