...
func NormalizeDriveCategory(text string, mediaType *domain.MediaType) *domain.DriveCategory {
	...
	// ⚡ Bolt: Replace sequential strings.ReplaceAll calls with a single-pass
	// byte-level iteration using strings.Builder to eliminate intermediate allocations.
	// A fast-path check avoids allocations if no target characters are present.
	compact := folded
