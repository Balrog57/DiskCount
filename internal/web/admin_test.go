...
func TestCSRFProtection(t *testing.T) {
	...
	tests := []struct {
		...
		{"GET Request", "GET", "http://evil.com", "", "localhost", http.StatusOK},
		{"POST Missing Headers", "POST", "", "", "localhost", http.StatusOK},
		{"POST Valid Origin", "POST", "http://localhost", "", "localhost", http.StatusOK},
		{"POST Invalid Origin", "POST", "http://evil.com", "", "localhost", http.StatusForbidden},
		{"POST Null Origin", "POST", "null", "", "localhost", http.StatusForbidden},
		{"POST Valid Referer", "POST", "", "http://localhost/path", "localhost", http.StatusOK},
		{"POST Invalid Referer", "POST", "", "http://evil.com/path", "localhost", http.StatusForbidden},
		{"POST Similar Host", "POST", "http://evillocalhost", "", "localhost", http.StatusForbidden},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			req := httptest.NewRequest(tc.method, "/login", strings.NewReader("password=wrong"))
			req.Host = tc.host
			req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
			if tc.origin != "" {
			...
			if rec.Code != tc.expectedCode {
				t.Errorf("expected %d, got %d", tc.expectedCode, rec.Code)
			}
		})
	}
}
